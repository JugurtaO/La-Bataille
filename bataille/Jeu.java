 ...
 *
 * Contient le main.
 * Vous ne pouvez lancer le main que lorsque toutes les classes ont été
 * complétées
 ...
 * Cette méthode affiche la carte carte sur le label de la carte gauche
 * (carteJoueurGaucheLabel)
 * si joueur == PlaceJoueur.GAUCHE ou sur le label de la carte droite
 * (carteJoueurDroiteLabel) si
 * joueur == PlaceJoueur.DROITE.
 * Tip: pour afficher une carte sur un label label, il faut utiliser la méthode
 * label.setIcon(new ImageIcon(fileName)) où fileName est le nom du fichier où
 * trouver
 * l'image correspondant à la carte. Attention, fileName doit contenir
 * PATH_TO_IMAGES.
 *
 * 2- la méthode jouer()
 ...
 * Cette méthode est la méthode appelée lorsque l'on clique sur le bouton
 * remporter.
 * Cette méthode implément ce qu'il se passe lorsque les 2 joueurs ont joués et
 * que l'un d'eux remporte.
 * - Appel la méthode gagnant() de la classe Bataille
 * - S'il y a bataille, écrit un commentaire "Bataille ! on rejoue..."
 * - Sinon, écrit "C'est le joueur nomDujoueurQuiRemporte qui remporte le pli."
 * et
 * vide le set de jeu de ses cartes.
 *
 * Tip: pour écrire un commentaire, utiliser la méthode setText(s: String)
 * de la classe JLabel sur l'attribut commentaire.
 * Tip: pour effacer une carte, il suffit de faire un setIcon(null) sur le label
 * correspondant.
 *
 * @author cfouard
 */
public class Jeu {
    ...
    void jouer() {
        ...
        commentaire.setText(bataille.joueurGauche.nom + "joue un(e) " + bataille.joueurGauche.carte.toString() + "\n"+ bataille.joueurDroite.nom + "joue un(e) " + bataille.joueurDroite.carte.toString());
    ...
    void remporter() {
        ...
        if (gagnant != null) {
            commentaire.setText("C'est le joueur " + bataille.gagnant().nom + "qui remporte le pli.");
